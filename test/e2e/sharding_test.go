...
func TestShardsReconcileAssignedApps(t *testing.T) {
	...
	RestartProcess(ApplicationControllerProcName, env)
...
func createClusterSecretWithShard(ctx *Context, shard int, ns string) string {
	...
	_, err = KubeClientset.RbacV1().Roles(role.Namespace).Create(ctx.T().Context(), &role, metav1.CreateOptions{})
	require.NoError(ctx.T(), err)
	...
	_, err = KubeClientset.RbacV1().RoleBindings(ns).Create(ctx.T().Context(), &roleBinding, metav1.CreateOptions{})
	require.NoError(ctx.T(), err)
